...
function tableFor(journal, event) {
    ...
    for (let i = 0; i < journal.length; i++) {
        let t = 0;
        if (hasEvent(journal[i], event)) t += 1;
        if (journal[i].squirrel) t += 2;
        ++table[t];
    }
    return table;
}

console.log(tableFor(JOURNAL, 'pizza'));
...
function gatherCorrelation(journal) {
    ...
    for (let i = 0; i < journal.length; i++) {
        ...
        for (let j = 0; j < events.length; j++) {
            ...
            if (!(event in phis)) {
                phis[event] = fi(tableFor(journal, event));
            }
        }
    }
    return phis;
...
for (const event in correlation) {
    if (Math.abs(correlation[event]) > 0.1) {
        console.log(event, correlation[event]);
    }
}

console.log([1, 2].shift());
console.log("asdfghsdfg".indexOf("df"));
console.log("asdfghsdfg".lastIndexOf("df"));
console.log("asdfghsdfg".slice(4, 7));
console.log("alex"[2]);