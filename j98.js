...
const persons = JSON.parse(ANCESTRY_FILE);
console.log(persons.length);

function _filter(array, checker) {
    const result = [];
    array.forEach(function (p) {
        if (checker(p)) result.push(p);
    });
    return result;
}

console.log('_filter', _filter(persons, function (p) {
    return p.born >= 1900;
}));
...
console.log([1, 2, 3, 4, 5].reduce(function (a1, a2) {
    return a1 + a2;
}, -1));

console.log('empty array', [].reduce(function (a1, a2) {
    return a1 + a2;
}, 23));

console.log('oldest', persons.reduce(function (p1, p2) {
    return p1.born < p2.born ? p1 : p2;
}));
...
function avg(ages) {
    ...
    return ages.reduce(plus) / ages.length;
}
...
console.log(avg(persons.filter(isMale).map(age)));
console.log(avg(persons.filter(isFemale).map(age)));
...
console.log(reduceAncestors(byName["Jan Haverbeke"], sharedDNA, 0) / 4);
console.log(reduceAncestors(byName["Lieven van Haverbeke"], sharedDNA, 0) / 4);

function countAncestors(person, test) {
    function combine(person, fromMother, fromFather) {
        return fromMother + fromFather + (test(person) ? 1 : 0);
    }
...
function longLivingAncestors(person) {
    const all = countAncestors(person, function (person) {
        return true;
    });
    const longLiving = countAncestors(person, function (person) {
        return age(person) >= 70;
    });
...
function _plus(q1, q2) {
    console.log(this);
    return q1 + q2;
}

console.log(_plus.bind('e', 2)(3));
console.log(_plus.bind(null, 7, 8)());
console.log(_plus.apply(null, [4, 5]));
...
function _groupBy(array, keyExtractor) {
    const result = {};
...
const byCenturies = _groupBy(persons, function (p) {
    return Math.ceil(p.died / 100);
});

for (let c in byCenturies) {
    console.log('avg age in ', c, ' == ', avg(byCenturies[c].map(age)))
}